...
//        String verificationToken = jwtService.generateJwtToken(request.getEmail());
        ...
        if (redisTemplate.hasKey("otp:" + request.getEmail())) {
    ...
    @Override
    public List<UserResponse> getAllOtherUsers(String userEmail) {

        User user = userRepository.findByEmail(userEmail).orElseThrow(
                () -> new RuntimeException("User not found with email: " + userEmail)
        );
        return userRepository.findAllOtherUsers(user.getUserId()).stream().map(this::mapToUserResponse).collect(Collectors.toList());
    }

    @Override
    public Optional<UserResponse> getMyProfile(String userEmail) {
        User user = userRepository.findByEmail(userEmail).orElseThrow(
                () -> new RuntimeException("User not found with email: " + userEmail)
        );
        return userRepository.findByEmail(user.getEmail()).map(this::mapToUserResponse);
    }

    @Override
    public List<UserResponse> getAllMembersInGroups(Long groupId) {
        List<User> memberInGroups = userRepository.findAllMembersInGroupDB(groupId);
